    ...
    for ((
        ...
        if capturable:
            ...
            if amsgrad:
                ...
                torch._foreach_copy_(device_grads, device_max_exp_avg_sqs)
            else:
                torch._foreach_copy_(device_grads, device_exp_avg_sqs)

            torch._foreach_sqrt_(device_grads)
            torch._foreach_div_(device_grads, bias_correction2_sqrt)
            torch._foreach_add_(device_grads, eps)
            torch._foreach_div_(device_grads, step_size)

            # at this point, exp_avg_sq_sqrt = - (1 - beta^t) * [sqrt(exp_avg_sq / (1 - beta2^t)) + eps] / lr
            torch._foreach_addcdiv_(device_params, device_exp_avgs, device_grads)
        else:
            ...
            torch._foreach_div_(device_grads, bias_correction2_sqrt)
            torch._foreach_add_(device_grads, eps)
            torch._foreach_addcdiv_(device_params, device_exp_avgs, device_grads, step_size)
